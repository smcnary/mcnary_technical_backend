...
const google_places_1 = require("../adapters/google-places");
...
const orchestrator = new orchestrator_1.CampaignOrchestrator();
const googlePlacesApiKey = process.env.GOOGLE_PLACES_API_KEY;
if (googlePlacesApiKey) {
    const googlePlacesAdapter = new google_places_1.GooglePlacesAdapter(googlePlacesApiKey);
...
const validateCampaignSpec = (req, res, next) => {
    const { body } = req;
    ...
    if (!Object.values(canonical_1.Vertical).includes(body.vertical)) {
        res.status(400).json({
            error: `Invalid vertical. Must be one of: ${Object.values(canonical_1.Vertical).join(', ')}`
        });
        return;
    }
...
app.get('/v1/verticals', (req, res) => {
    res.json({
        verticals: Object.values(canonical_1.Vertical).map((v) => ({
            value: v,
...
app.get('/v1/leads', async (req, res) => {
    try {
        ...
        if (runId) {
            csvFiles = csvFiles.filter(file => file.includes(runId));
        }
        if (campaign) {
            csvFiles = csvFiles.filter(file => file.toLowerCase().includes(campaign.toLowerCase()));
        }
        ...
        if (search) {
            ...
            filteredLeads = filteredLeads.filter(lead => lead.company_name?.toLowerCase().includes(searchTerm) ||
...
function parseCSVToLeads(csvContent, filename) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (lead.lead_score)
            lead.lead_score = parseFloat(lead.lead_score);
        if (lead.rating)
            lead.rating = parseFloat(lead.rating);
        if (lead.review_count)
            lead.review_count = parseInt(lead.review_count);
        if (lead.latitude)
            lead.latitude = parseFloat(lead.latitude);
        if (lead.longitude)
            lead.longitude = parseFloat(lead.longitude);
        if (lead.founded_year)
            lead.founded_year = parseInt(lead.founded_year);
        leads.push(lead);
    }
    return leads;
}
...
app.get('/v1/campaigns/example', (req, res) => {
    const exampleCampaign = {
        ...
        vertical: canonical_1.Vertical.LOCAL_SERVICES,
        geo: {
            city: 'Denver',
            region: 'CO',
            country: 'US',
