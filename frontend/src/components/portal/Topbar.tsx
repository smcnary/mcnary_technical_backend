...
export default function Topbar() {
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      try {
        console.log('🔄 Topbar: Loading user profile...');
        
        // First test API connection
        console.log('🧪 Topbar: Testing API connection...');
        const apiWorking = await testApiConnection();
        console.log('🧪 Topbar: API connection test result:', apiWorking);
        
        if (!apiWorking) {
          console.error('❌ Topbar: API connection test failed');
          setIsLoading(false);
          return;
        }
        
        const profile = await fetchUserProfile();
        console.log('✅ Topbar: User profile loaded:', profile);
        console.log('👤 Topbar: User data:', profile.user);
        console.log('📧 Topbar: Email:', profile.user.email);
        console.log('👤 Topbar: First Name:', profile.user.firstName);
        console.log('👤 Topbar: Last Name:', profile.user.lastName);
        setUserProfile(profile);
      } catch (error) {
        console.error('❌ Topbar: Failed to load user profile:', error);
